...
# -*- coding: utf-8 -*-
"""
Softcam Manager - Gestion des lignes CCcam et Oscam
"""
...
class SoftcamHandler(BaseHTTPRequestHandler):
    ...
    def _delete_line(self, lid):
        ...
        if self._save_lines(new_lines):
            self._send_json(200, {"success": True, "message": f"Ligne supprimée"})
        else:
    ...
    def _save_lines(self, lines):
        try:
            ...
            with open(CCCAM_FILE, 'w') as f:
                f.write("# Generated by Softcam Manager\n\n")
                if cccam:
                    for l in cccam:
...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; font-family:system-ui, 'Segoe UI', Roboto, sans-serif; }
        ...
        .stat { background:rgba(255,255,255,0.05); border-radius:20px; padding:15px 30px; text-align:center; flex:1; }
        .stat-value { font-size:2rem; font-weight:700; color:#60efff; }
        .stat-label { color:#a8b2d1; margin-top:6px; font-size:0.85rem; }
        .tabs { display:flex; gap:10px; margin-bottom:30px; }
        .tab { padding:10px 25px; background:rgba(255,255,255,0.05); border-radius:50px; cursor:pointer; font-weight:600; }
        ...
        .items-list { margin-top:20px; display:flex; flex-direction:column; gap:12px; }
        .item-card { background:rgba(0,0,0,0.35); border-radius:16px; padding:16px 20px; border-left:4px solid; }
        .item-card.cccam { border-left-color:#f59e0b; }
        .item-card.oscam { border-left-color:#8b5cf6; }
        .item-header { display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px; margin-bottom:10px; }
        ...
        .item-details { font-size:0.8rem; color:#9ca3af; margin:8px 0; }
        .item-actions { display:flex; gap:8px; margin-top:12px; flex-wrap:wrap; }
        .alert { position:fixed; bottom:20px; right:20px; background:#1e293b; border-left:4px solid; padding:14px 20px; border-radius:16px; color:white; z-index:1000; opacity:0; transition:0.3s; pointer-events:none; }
        ...
        .modal-content { background:#1e1b4b; border-radius:28px; width:90%; max-width:500px; padding:28px; }
...
<div class="container">
    <div class="card">
        <div class="card-header"><h2>🔑 Gestion des lignes Softcam (CCcam / Oscam)</h2></div>
        <div class="card-body">
            <div class="stats">
            ...
            <div class="tabs">
                <div class="tab active" onclick="showTab('cccam')">📡 CCcam (C: host port user pass)</div>
                <div class="tab" onclick="showTab('oscam')">⚙️ Oscam ([reader])</div>
            </div>

            <!-- Formulaire CCcam -->
            <div id="formCccam" class="form-panel active">
                <div class="form-grid">
                    <div><label>🌐 Hôte / Serveur</label><input type="text" id="hostCCcam" placeholder="ex: server.dyndns.org"></div>
                    <div><label>🔌 Port</label><input type="text" id="portCCcam" placeholder="12000"></div>
                    <div><label>👤 Nom d'utilisateur</label><input type="text" id="userCCcam" placeholder="username"></div>
                    <div><label>🔒 Mot de passe</label><input type="password" id="passCCcam" placeholder="password"></div>
                </div>
                <div class="checkbox-row"><input type="checkbox" id="activeCCcam" checked> <label>✅ Activer cette ligne</label></div>
                <button class="btn btn-warning" onclick="addLine('cccam')" style="margin-top:20px">➕ Ajouter ligne CCcam</button>
            </div>

            <!-- Formulaire Oscam -->
            <div id="formOscam" class="form-panel">
                <div class="form-grid">
                    <div><label>🌐 Hôte / Serveur</label><input type="text" id="hostOscam" placeholder="ex: server.dyndns.org"></div>
                    <div><label>🔌 Port</label><input type="text" id="portOscam" placeholder="12000"></div>
                    <div><label>👤 Nom d'utilisateur</label><input type="text" id="userOscam" placeholder="username"></div>
                    <div><label>🔒 Mot de passe</label><input type="password" id="passOscam" placeholder="password"></div>
                </div>
                <div class="checkbox-row"><input type="checkbox" id="activeOscam" checked> <label>✅ Activer cette ligne</label></div>
                <button class="btn btn-purple" onclick="addLine('oscam')" style="margin-top:20px">➕ Ajouter ligne Oscam</button>
            </div>

            <hr>
            <h3 style="margin:20px 0 15px 0;">📋 Liste des lignes configurées</h3>
            <div id="linesList"><div class="loading" style="text-align:center; padding:30px;">⏳ Chargement...</div></div>
        </div>
    </div>
</div>
...
<script>
    function showTab(tab) {
        ...
        if(tab === 'cccam') {
            ...
            document.getElementById('formCccam').classList.add('active');
            document.getElementById('formOscam').classList.remove('active');
        } else {
            document.querySelector('.tab:last-child').classList.add('active');
            document.getElementById('formOscam').classList.add('active');
            document.getElementById('formCccam').classList.remove('active');
        }
    }
    ...
    function renderLines(lines) {
        ...
        if(!lines.length) {
            container.innerHTML = '<div class="loading">📭 Aucune ligne configurée</div>';
            return;
        }
        container.innerHTML = lines.map(l => `
            <div class="item-card ${l.type}">
                ...
                <div class="item-details">👤 ${escapeHtml(l.username)}</div>
                <div class="item-actions">
                    <button class="btn btn-sm ${l.active ? 'btn-warning' : 'btn-success'}" onclick="toggleActive('${l.id}', ${!l.active})">${l.active ? '⏸️ Désactiver' : '▶️ Activer'}</button>
                    <button class="btn btn-sm btn-purple" onclick="openEditModal('${l.id}')">✏️ Modifier</button>
                    <button class="btn btn-sm btn-danger" onclick="deleteLine('${l.id}')">🗑️ Supprimer</button>
                </div>
            </div>
        `).join('');
    }
    ...
    async function addLine(type) {
        let data = { type };
        if(type === 'cccam') {
            data.host = document.getElementById('hostCCcam').value.trim();
            data.port = document.getElementById('portCCcam').value.trim();
            data.username = document.getElementById('userCCcam').value.trim();
            data.password = document.getElementById('passCCcam').value.trim();
            data.active = document.getElementById('activeCCcam').checked;
        } else {
            data.host = document.getElementById('hostOscam').value.trim();
            data.port = document.getElementById('portOscam').value.trim();
            data.username = document.getElementById('userOscam').value.trim();
            data.password = document.getElementById('passOscam').value.trim();
            data.active = document.getElementById('activeOscam').checked;
        }
        ...
        try {
            ...
            if(d.success) {
                ...
                if(type === 'cccam') {
                    document.getElementById('hostCCcam').value = '';
                    document.getElementById('portCCcam').value = '';
                    document.getElementById('userCCcam').value = '';
                    document.getElementById('passCCcam').value = '';
                    document.getElementById('activeCCcam').checked = true;
                } else {
                    document.getElementById('hostOscam').value = '';
                    document.getElementById('portOscam').value = '';
                    document.getElementById('userOscam').value = '';
                    document.getElementById('passOscam').value = '';
                    document.getElementById('activeOscam').checked = true;
                }
                loadLines();
            } else {
